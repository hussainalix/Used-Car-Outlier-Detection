...
The Solution: IQR MethodI implemented the Interquartile Range (IQR) method to handle these outliers mathematically:Lower Quartile ($Q1$): 25th percentile of the data.Upper Quartile ($Q3$): 75th percentile of the data.IQR Formula: $IQR = Q3 - Q1$Boundaries: - Lower: $Q1 - 1.5 \times IQR$Upper: $Q3 + 1.5 \times IQR$
